...
| `log-observation`, `record-outcome`, `behavior-summary` | behavior_log | Core observation logging — subject, category, severity, verified-or-asserted, plus outcome tracking after cross-examination. Optional `--decision-type` (continuation_approval/directive_change, section 7.5 of the ledger security spec) and `--token-metrics` (validated JSON: input/output/cached_tokens, files_opened -- deliberately excludes any computed quality score, see `docs/TOKEN_EFFICIENCY_AND_ROUTING.md`) both also work on `propose-observation`, carrying through `approve-pending`. |
...
| `propose-observation`, `review-pending`, `approve-pending`, `reject-pending`, `supersede-pending` | behavior_log | The real quarantine boundary: a free-tier/sandboxed instance can stage an observation, but only an explicit human command commits it to the real ledger. `supersede-pending` (added 2026-08-25) is the third terminal path -- a good-faith proposal overtaken by stronger evidence, not false (would misstate it) and not promoted to VERIFIED-tier (would overstate it). All three terminal transitions are idempotent and mutually exclusive -- a proposal can't end up both approved and rejected under crash-recovery conditions. |
